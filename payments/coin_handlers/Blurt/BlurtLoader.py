...
class BlurtLoader(BaseLoader, BlurtMixin):
    ...
    def __init__(self, symbols):
        super(BlurtLoader, self).__init__(symbols=symbols)
        self.tx_count = 100
        self.loaded = False
        self._rpc = None
        self._rpcs = {}
    ...
    def load(self, tx_count=1000):
        ...
        # Thus the default TX count for Blurt is 1000
    ...
    def clean_txs(self, symbol: str, transactions: Iterable[dict], account: str = None) -> Generator[dict, None, None]:
        ...
        for tx in transactions:
            try:
                ...
                log.warning('Blurt TX missing important key? %s', str(e))
            except:
                log.exception('Error filtering Blurt TX, skipping... TX data: %s', tx)
    ...
    def clean_tx(self, tx: dict, symbol: str, account: str, memo: str = None, memo_case: bool = False) -> Union[dict, None]:
        ...
        if tx.get('type', 'NOT SET') != 'transfer':
            log.debug('Blurt TX is not transfer. Type is: %s', tx.get('type', 'NOT SET'))
            return None
        ...
        log.debug('Filtering/cleaning Blurt transaction, Amt: %f, TXID: %s', amt, txid)
