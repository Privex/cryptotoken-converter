...
from payments.coin_handlers.Blurt import BlurtLoader
from payments.coin_handlers.Blurt.BlurtMixin import BlurtMixin
...
class BlurtManager(BaseManager, BlurtMixin):
    provides = ["BLURT"]  # type: List[str]
    """
    This attribute is automatically generated by scanning for :class:`models.Coin` s with the type ``blurtbase``.
    This saves us from hard coding specific coin symbols. See __init__.py for populating code.
    """

    def __init__(self, symbol: str):
        super(BlurtManager, self).__init__(symbol)
    ...
    def health_test(self) -> bool:
        """
        Check if our Blurt node works or not, by requesting basic information such as the current block + time, and
        checking if our sending/receiving account exists on Blurt.

        :return bool: True if Blurt appears to be working, False if it seems to be broken.
        """
        try:
    ...
    def address_valid(self, address) -> bool:
        """
        If an account exists on Blurt, will return True. Otherwise False.

        :param address: Blurt account to check existence of
    ...
    def balance(self, address: str = None, memo: str = None, memo_case: bool = False) -> Decimal:
        """
        Get token balance for a given Blurt account, if memo is given - get total symbol amt received with this memo.

        :param address:    Blurt account to get balance for, if not set, uses self.coin.our_account
        ...
        if not empty(memo):
            ...
            total = Decimal(0)
            s = BlurtLoader(symbols=[self.symbol])
            for h in hist:
